...
Expression Parser::ParseSimpleExpression() {
    ...
    if (token == TOKENS[OPBR]) { // "("
        auto result = Parse();
        if (ParseToken() != TOKENS[CLBR]) // ")"
...
double Solve(const Expression &expr) {
    switch (expr.args.size()) {
        ...
        case 1: {
            ...
            if (expr.token == TOKENS[TAN]) return tan(num);
            if (expr.token == TOKENS[ATAN]) return atan(num);
            if (expr.token == TOKENS[LN]) return log(num);
            if (expr.token == TOKENS[LG]) return log10(num);
